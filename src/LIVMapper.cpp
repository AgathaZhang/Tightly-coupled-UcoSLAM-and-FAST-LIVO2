      ...
      extR(M3D::Identity())
{
  ...
  readParameters(nh);
  VoxelMapConfig voxel_config;    // 声明体素配置
  loadVoxelConfig(nh, voxel_config);

  visual_sub_map.reset(new PointCloudXYZI());   // PointCloudXYZI::Ptr
  ...
  vio_manager->set_cam_model_string(cam_model_string);
  root_dir = ROOT_DIR;
  initializeFiles();
  initializeComponents(); // 用配置参数初始化体素地图、VIO 管理器、IMU 模块的外参和核心参数，并根据传感器组合确定当前 SLAM 模式（LIVO / ONLY_LIO / ONLY_LO）
  path.header.stamp = ros::Time::now();
  path.header.frame_id = "camera_init";


   //******************** backend  start  self ******************************
        client_search = nh.serviceClient<fast_livo::ImagePoseTime>("/image_retrieval");
        pub_odom_finish_str_msg = nh.advertise<std_msgs::String>("/loop_detect_finish", 0);
        //client_insert = m_ros_node_handle.serviceClient<r3live::InsertImagePoseTime>("/insert_database");
        downSizeFilterMap_loopdetect.setLeafSize(0.05, 0.05, 0.05);


        nh.param< int >( "r3live_lio/loopdetect_enable", loopdetect_enable,1 );
        nh.param< int >( "r3live_lio/loopdetect_enable_just_final", loopdetect_enable_just_final,0 );
        nh.param< int >( "r3live_lio/use_first_localization_constraint", use_first_localization_constraint,1 );

        
        nh.param< float >( "r3live_lio/loopdetect_scale", ld_loopdetect_scale,1.0 );
        nh.param< float >( "r3live_lio/map_icp_teaser_scale", ld_map_icp_teaser_scale,30 );
        nh.param< int >( "r3live_lio/use_map_icp_teaser", ld_use_map_icp_teaser,0 );
        nh.param< float >( "r3live_lio/downSizeFilterMap_loopdetect_ds", downSizeFilterMap_loopdetect_ds,0.05 );
        nh.param< float >( "r3live_lio/surroundingkeyframeAddingAngleThreshold", ld_surroundingkeyframeAddingAngleThreshold,0.2 );
        nh.param< float >( "r3live_lio/surroundingkeyframeAddingDistThreshold", ld_surroundingkeyframeAddingDistThreshold,0.5 );
        nh.param< float >( "r3live_lio/min_pos_keyframe_color_cloud", ld_min_pos_keyframe_color_cloud,0.25 );
        nh.param< float >( "r3live_lio/min_rot_keyframe_color_cloud", ld_min_rot_keyframe_color_cloud,10 );
        nh.param< int >( "r3live_lio/historyKeyframeSearchNum_target", ld_historyKeyframeSearchNum_target,40 );
        nh.param< int >( "r3live_lio/historyKeyframeSearchNum_source", ld_historyKeyframeSearchNum_source,20 );
        nh.param< int >( "r3live_lio/historyKeyframeSearchNum_target_finish", ld_historyKeyframeSearchNum_target_finish,20 );
        nh.param< int >( "r3live_lio/historyKeyframeSearchNum_source_finish", ld_historyKeyframeSearchNum_source_finish,20 );
        nh.param< float >( "r3live_lio/last_loop_detect_time_diff_threshold", ld_last_loop_detect_time_diff_threshold,60 );

        nh.param< float >( "r3live_lio/ld_inlier_value", ld_inlier_value,0.4 );
        nh.param< float >( "r3live_lio/ld_inlier_value_finish", ld_inlier_value_finish,0.4 );


        nh.param< float >( "r3live_lio/ld_historyKeyframeSearchRadius", ld_historyKeyframeSearchRadius,2 );
        nh.param< float >( "r3live_lio/ld_historyKeyframeSearchTimeDiff", ld_historyKeyframeSearchTimeDiff,60 );


        std::cout<<"==============loop detect param=================="<<std::endl;
        std::cout<<"loopdetect_enable:"<<loopdetect_enable<<std::endl;
        std::cout<<"loopdetect_enable_just_final:"<<loopdetect_enable_just_final<<std::endl;
        std::cout<<"loopdetect_scale:"<<ld_loopdetect_scale <<std::endl;
        std::cout<<"map_icp_teaser_scale:"<< ld_map_icp_teaser_scale<<std::endl;
        std::cout<<"use_map_icp_teaser:"<<ld_use_map_icp_teaser <<std::endl;
        std::cout<<"downSizeFilterMap_loopdetect_ds:"<<downSizeFilterMap_loopdetect_ds <<std::endl;
        std::cout<<"surroundingkeyframeAddingAngleThreshold:"<<ld_surroundingkeyframeAddingAngleThreshold <<std::endl;
        std::cout<<"surroundingkeyframeAddingDistThreshold:"<< ld_surroundingkeyframeAddingDistThreshold<<std::endl;
        std::cout<<"min_pos_keyframe_color_cloud:"<<ld_min_pos_keyframe_color_cloud <<std::endl;
        std::cout<<"min_rot_keyframe_color_cloud:"<<ld_min_rot_keyframe_color_cloud <<std::endl;
        std::cout<<"historyKeyframeSearchNum_target:"<<ld_historyKeyframeSearchNum_target <<std::endl;
        std::cout<<"historyKeyframeSearchNum_source:"<< ld_historyKeyframeSearchNum_source<<std::endl;
        std::cout<<"historyKeyframeSearchNum_target_finish:"<<ld_historyKeyframeSearchNum_target_finish <<std::endl;
        std::cout<<"historyKeyframeSearchNum_source_finish:"<<ld_historyKeyframeSearchNum_source_finish <<std::endl;
        std::cout<<"last_loop_detect_time_diff_threshold:"<<ld_last_loop_detect_time_diff_threshold <<std::endl;

        std::cout<<"ld_inlier_value:"<<ld_inlier_value<<std::endl;
        std::cout<<"ld_inlier_value_finish:"<<ld_inlier_value_finish<<std::endl;
        std::cout<<"ld_historyKeyframeSearchRadius:"<<ld_historyKeyframeSearchRadius<<std::endl;
        std::cout<<"ld_historyKeyframeSearchTimeDiff:"<<ld_historyKeyframeSearchTimeDiff<<std::endl;

        std::cout<<"use_first_localization_constraint:"<<use_first_localization_constraint<<std::endl;
        std::cout<<"=============loop detect param==================="<<std::endl;


        if(USE_LOOP_PARAM){
          downSizeFilterMap_loopdetect.setLeafSize(downSizeFilterMap_loopdetect_ds, downSizeFilterMap_loopdetect_ds, downSizeFilterMap_loopdetect_ds);
        }

        //ceresIcpTool_handle = new kilox::CeresIcpTool(0.4,0.4);//(0.4,0.4);
        feats_undistort_r3live.reset( new pcl::PointCloud<pcl::PointXYZINormal>() );
        feats_undistort_imu_r3live.reset( new pcl::PointCloud<pcl::PointXYZINormal>() );
        
        current_to_new_transform.R_ = Eigen::Matrix3d::Identity();
        current_to_new_transform.t_ = Eigen::Vector3d::Zero();
        cloud_temp_down.reset(new pcl::PointCloud<pcl::PointXYZINormal>());
        ...
        cloudKeyPoses6D_test.reset(new pcl::PointCloud<PointTypePose>());
        copy_cloudKeyPoses3D.reset(new pcl::PointCloud<pcl::PointXYZINormal>());
        copy_cloudKeyPoses6D.reset(new pcl::PointCloud<PointTypePose>());
        
        kdtreeHistoryKeyPoses.reset(new pcl::KdTreeFLANN<pcl::PointXYZINormal>());

        subMapKeyFrames.reset(new pcl::PointCloud<pcl::PointXYZINormal>());
        subMapKeyPoses.reset(new pcl::PointCloud<pcl::PointXYZINormal>());
        subMapKeyPosesDS.reset(new pcl::PointCloud<pcl::PointXYZINormal>());
        subMapKeyFramesDS.reset(new pcl::PointCloud<pcl::PointXYZINormal>());

        downSizeFilterICP.setLeafSize(0.2, 0.2, 0.2);

        downSizeFilterICP_hba.setLeafSize(0.01, 0.01, 0.01);

        fastlio_unoptimized_cloudKeyPoses6D = pcl::PointCloud<PointTypePose>::Ptr((new pcl::PointCloud<PointTypePose>())); //  存储fastlio 未优化的位姿
        gnss_cloudKeyPoses6D = pcl::PointCloud<PointTypePose>::Ptr((new pcl::PointCloud<PointTypePose>())); //  gnss 轨迹
        // 初始化gtsam的求解器
        // ISAM2参数
        ...
        for (int i = 0; i < 6; ++i)
        {
            transformTobeMapped[i] = 0;   // [roll, pitch, yaw, x, y, z] 初始化为0位姿
        }
        
        last_search_img_time = 0;
        pubLoopDetectSource = nh.advertise<sensor_msgs::PointCloud2>("/loopdetect_source", 100);
        pubLoopDetectSourceTransform = nh.advertise<sensor_msgs::PointCloud2>("/loopdetect_source_transform", 100);
        pubLoopDetectTarget = nh.advertise<sensor_msgs::PointCloud2>("/loopdetect_target", 100);
        loop_thread_boot = std::make_shared< std::thread >( &LIVMapper::loopClosureThread, this );
        //******************** backend  end ******************************
}
...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  // nh.param<string>("common/img_topic", img_topic, "/left_camera/image"); // 临时注释掉

  nh.param<string>("/usb_cam_left/image_raw/compressed", img_topic, "/usb_cam_left/image_raw/compressed");
  // nh.param<string>("/usb_cam_left/image_raw/compressed", img_topic_left_compressed/*LIVMapper*/, "/usb_cam_left/image_raw/compressed");
  // nh.param<string>("/usb_cam_right/image_raw/compressed", img_topic_right_compressed, "/usb_cam_right/image_raw/compressed");

  nh.param<string>("common/cam_model", cam_model_string, "Pinhole");
  std::cout<<"cam_model_string:"<<cam_model_string<<std::endl;
  ...
  // nh.param<double>("preprocess/img_type", p_pre->img_type, "compressed");
  ...
  nh.param<string>("common/full_camera_trajectory", full_camera_trajectory, "/home/kilox/wutong/camera_full_trajectory.txt");
...
void LIVMapper::initializeSubscribersAndPublishers(ros::NodeHandle &nh, image_transport::ImageTransport &it) 
{
  ...
  // if (p_pre->img_type == "compressed") {
  //   std::cout<< "======> 1" << std::endl;
  //   sub_img = nh.subscribe(img_topic, 200000, &LIVMapper::img_compressed_cbk, this);
  // } else {
  //   std::cout<< "======> 2" << std::endl;
  //   sub_img = nh.subscribe(img_topic, 200000, &LIVMapper::img_cbk, this);
  // }
  sub_img = p_pre->img_type == "compressed" ?
            nh.subscribe(img_topic, 200000, &LIVMapper::img_compressed_cbk, this):  // 条件表达式判断
...
void LIVMapper::handleFirstFrame() // 07.18 午
{
  if (!is_first_frame)
  {
...
void LIVMapper::processImu() 
{
  ...
  p_imu->Process2(LidarMeasures, _state, feats_undistort);// 执行 IMU 初始对齐、IMU 预积分、点云去畸变UndistortPcl 在第一次运行时触发 IMU 初值估计（重力、bias），多帧迭代

  if (gravity_align_en) gravityAlignment();   // 将状态向量中的重力、位姿、速度旋转到一致的重力方向，保证世界系一致性
...
void LIVMapper::handleVIO() // 07.21 午
{
  euler_cur = RotMtoEuler(_state.rot_end);    // 计算当前欧拉角
...
/** 对特征点做跟踪、投影、残差构建，然后更新 EKF 的状态估计*/
//开启vio.cpp中的进程
  vio_manager->processFrame(LidarMeasures.measures.back().img,    // 最新的图像帧 measures.back().img
                            _pv_list,                             // 当前点云关联的点列表 _pv_list
                            voxelmap_manager->voxel_map_,         // 当前 voxel 地图 voxelmap_manager->voxel_map_
                            LidarMeasures.last_lio_update_time - _first_lidar_time);  // 同步时间戳 LidarMeasures.last_lio_update_time - _first_lidar_time
...
/** 从这里开始*/
  EigenPose camera2world;
  Eigen::Quaterniond camera2world_q(vio_manager->new_frame_->T_f_w_.rotation_matrix());
  Eigen::Vector3d camera2world_t = vio_manager->new_frame_->T_f_w_.translation();
  EigenPose world2camera(camera2world_q, camera2world_t);

  camera2world.R_ = world2camera.inverse().R_;
  camera2world.t_ = world2camera.inverse().t_;

  double current_img_timestamp = LidarMeasures.measures.back().vio_time - img_time_offset;//img_time_offset*2;
  double last_lidar_timestamp =  last_lidar_pose.time;
  Eigen::Vector3d pos_lidar(last_lidar_pose.x, last_lidar_pose.y, last_lidar_pose.z);
  Eigen::Quaterniond q_lidar = EulerToQuat(last_lidar_pose.roll, last_lidar_pose.pitch, last_lidar_pose.yaw);
  EigenPose lidar_to_world_new(q_lidar, pos_lidar);
  //相机到上一个雷达关键帧的距离
  EigenPose camera2lidar_new = lidar_to_world_new.inverse() * camera2world;
  std::stringstream write_loop_bag_stream;
  //0是左目
  write_loop_bag_stream << std::setprecision (16)  << current_img_timestamp <<" " <<std::setprecision (16)<< last_lidar_timestamp << " " << camera2lidar_new.t_(0) << " "<<camera2lidar_new.t_(1) << " "<<camera2lidar_new.t_(2) << " "
                        << camera2lidar_new.GetQ().x() << " " << camera2lidar_new.GetQ().y() << " "
                        << camera2lidar_new.GetQ().z() << " " << camera2lidar_new.GetQ().w() << '\n';

  full_camera_trajectory_str += write_loop_bag_stream.str(); 
  //std::cout<<"time:"<<LidarMeasures.measures.back().vio_time<<std::endl;
  /**
  */
...
{    
  ...
  //******************** backend start **************
  feats_undistort_imu_r3live.reset(new pcl::PointCloud<pcl::PointXYZINormal>);
  transformLidar_to_IMU(feats_undistort, feats_undistort_imu_r3live);

  bool debug_print_result = true;
  //lidar_begin_global_time = LidarMeasures.lidar_frame_beg_time ;
  //std::cout<<"lidar_begin_global_time:"<<lidar_begin_global_time<<std::endl;
  //exit(0);
  updateCurPose(_state);
  //判断当前的帧是否适合添加进关键帧，适合则添加当前的关键帧到gtsam
  if(debug_print_result){
      std::cout<<"=========> saveKeyFramesAndFactor"<<std::endl;
  }
    
  bool is_result = saveKeyFramesAndFactor();

  if(is_result){
    pause_ros_bag();
    mtx_loopdetect_playbag.lock();
    mtx_loopdetect_playbag_flag = 0;
    mtx_loopdetect_playbag.unlock();
    while(mtx_loopdetect_playbag_flag == 0){
      usleep(1000*50);
    }
    play_ros_bag();
  }
...
//入口
/** pause_ros_bag();  // rosbag 播放状态控制 自动暂停或等待 避免数据堆积或等待*/

/** 加了 std::chrono 定时器，判断 lid_raw_data_buffer 缓冲区大小 + 播放时长 stay_total_time_bag，动态决定是否暂停或继续*/

/** 播放结束检测与收尾 用 check_rosbag_play_is_alive() 检查包是否播完，
 *  如果播完调用 performLoopClosureFinish() 做最后的回环优化
 *  写入完整的相机轨迹文件 outfile
 *  调用 saveKeyFramesAndFactorFinish() 保存关键帧和因子图
 *  调用 correctPosesFinish() 最终姿态修正*/

/** 循环中把 full_camera_trajectory_str 持续写入文件 full_camera_trajectory*/
void LIVMapper::run() 
{

  ros::Rate rate(5000);// 设置循环频率为5000Hz 是 ROS 用来控制 while 循环节奏的工具类
  

  std::chrono::time_point< std::chrono::system_clock > start_play_bag;
  std::chrono::time_point< std::chrono::system_clock > start_play_bag_fini;
  std::chrono::time_point< std::chrono::system_clock > start_play_bag_end;
  start_play_bag = std::chrono::system_clock::now();
  bool begin_pause = false;

  std::fstream outfile;
  std::string filename = full_camera_trajectory;
  outfile.open(filename, std::ios::out); //fixd outfile.open(filename, std::istream::out);
  full_camera_trajectory_str = "";

  bool has_data_process = false;
  while (ros::ok()) 
  {
    ros::spinOnce();// 处理回调队列中的消息
    //判断是否应该结束了
    std::chrono::duration<double> time_diff_bag = std::chrono::system_clock::now() - start_play_bag;
    double stay_total_time_bag = time_diff_bag.count();

    /** 这段逻辑是在实时监控 LiDAR 数据缓冲区大小，配合 rosbag 播放器，动态决定何时暂停 / 恢复
     *  rosbag 播放，并在 数据播完时执行整套“最后闭环 + 优化 + 轨迹保存 + 系统安全退出” 的完整收尾流程。*/
    if((lid_raw_data_buffer.size() > 100 && begin_pause == false)||(lid_raw_data_buffer.size() > 100)){
        pause_ros_bag();  // rosbag 播放状态控制 自动暂停或等待 避免数据堆积或等待
        start_play_bag = std::chrono::system_clock::now();
        begin_pause = true;
    }else if(has_data_process && ((lid_raw_data_buffer.size()<=1 && begin_pause == true)||(lid_raw_data_buffer.size()<=1 && (stay_total_time_bag>3)))){
        
        bool is_alive = check_rosbag_play_is_alive(); // 检查rosbag播放节点是否还活着

        if(is_alive){
          play_ros_bag();
        }
        std::cout<<"is_alive:"<<is_alive<<std::endl;
        if(is_alive == false){
            std::cout<<"当前包已经播放完毕zz，进行最后一次回环"<<std::endl;
            if(performLoopClosureFinish()){ // 系统结束前把最后的闭环信息补齐，让后端优化有闭环因子
                std::cout<<"===========> performLoopClosureFinish,now saveKeyFramesAndFactorFinish"<<std::endl;
                saveKeyFramesAndFactorFinish(); // 在系统结束时把闭环因子（Loop Factor）加入因子图，执行 ISAM 优化器多次迭代更新，得到更精确的全局关键帧轨迹估计，并把最新的位姿结果和协方差提取出来保存下来，为后续地图保存或回放提供最终一致的优化轨迹
            }
            ...
            outfile.flush();
            outfile.close();
            //std::cout<<"=======> full_camera_trajectory_str: "<<full_camera_trajectory_str<<std::endl;
            full_camera_trajectory_str = "";
            std::cout<<"===== system exit ====="<<std::endl;

            correctPosesFinish(); // 收尾阶段：把全局轨迹修正好、保存好、状态推送好，一切准备就绪可导出
            exit(0);              // 直接终止整个当前程序进程 正常退出
        }
        start_play_bag = std::chrono::system_clock::now();
        begin_pause = false;
    }
    //std::cout<<"line:"<<__LINE__<<" lidar buffer size:"<<lid_raw_data_buffer.size()<<" img_buffer:"<<img_buffer.size()<<" stay_total_time_bag: "<<stay_total_time_bag<<std::endl;
    //******************** backend end **************
    ...
    start_play_bag = std::chrono::system_clock::now();
    has_data_process = true;
  }
  savePCD();                            // 退出时保存点云地图(没用到)
}
...
void LIVMapper::standard_pcl_cbk(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  p_pre->process(msg, ptr);                           // 在这里根据不同雷达类型处理
...
{
  //ROS_WARN("IMU and LiDAR not synced! delta time");
  if (!imu_en) return;
...
void LIVMapper::img_compressed_cbk(const sensor_msgs::CompressedImageConstPtr& msg_comp)
{ // 这里修改了 cv::Mat msg_in = cv::imdecode(cv::Mat(msg_comp->data), 1);
  if (!img_en) return;

  // 用 cv_bridge 解压缩 深拷贝 + 格式转换
  cv_bridge::CvImagePtr cv_ptr = cv_bridge::toCvCopy(msg_comp, sensor_msgs::image_encodings::BGR8);

  // 这里 cv_ptr->header.stamp 继承了原始 compressed image 的 header
  double msg_header_time = cv_ptr->header.stamp.toSec() + img_time_offset;

  if (abs(msg_header_time - last_timestamp_img) < 0.001) return;    // XXX 帧太快返回
  ROS_INFO("Get image, its header time: %.6f", msg_header_time);
  if (last_timestamp_lidar < 0) return;                             // XXX 上一帧时间不对返回

  if (msg_header_time < last_timestamp_img)                         // XXX 帧回溯返回
  {
  ...
  if (img_time_correct - last_timestamp_img < 0.02)                 // XXX 帧率太高 解锁返回
  {
  ...
  // 直接使用 cv_ptr->image
  img_buffer.push_back(cv_ptr->image);                              // XXX 帧率正确 填充
...
void LIVMapper::img_cbk(const sensor_msgs::ImageConstPtr &msg_in)
{
  ...
  if (!img_en) return;
  sensor_msgs::Image::Ptr msg(new sensor_msgs::Image(*msg_in)); // 深拷贝 + 独立智能指针 完全独立 内存隔离
...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas)
{
  ...
  case LIVO:  /** BOTH LIO & VIO*/
  {
    ...
    case VIO:
    {
       ...
       * will be refresh. ***/
      if (meas.last_lio_update_time < 0.0) meas.last_lio_update_time = lid_header_time_buffer.front();  /** 取第一帧雷达为 最新*/
      // printf("[ Data Cut ] wait \n");
      // printf("[ Data Cut ] last_lio_update_time: %lf \n",
      // meas.last_lio_update_time);

      lidar_begin_global_time = lid_header_time_buffer.front();
      double lid_newest_time = lid_header_time_buffer.back() + lid_raw_data_buffer.back()->points.back().curvature / double(1000);  /** 最后雷达帧的完整采样时刻*/
      ...
      /** 如果图像的捕获时间晚于最新的激光雷达时间或最新的 IMU 时间 没得玩了*/
      ...
      while (!imu_buffer.empty())
      {
        if (imu_buffer.front()->header.stamp.toSec() > m.lio_time) break; // 如果当前IMU数据时间戳晚于当前LiDAR帧时间，说明当前IMU数据属于下一帧，停止处理

        if (imu_buffer.front()->header.stamp.toSec() > meas.last_lio_update_time) m.imu.push_back(imu_buffer.front());  // 如果IMU数据时间戳晚于上一次LIO更新时间，说明这部分IMU数据是当前帧需要的，加入当前帧IMU数据队列
      ...
      *(meas.pcl_proc_cur) = *(meas.pcl_proc_next); // 把当前处理帧 pcl_proc_next 的内容复制给上一帧 pcl_proc_cur
      PointCloudXYZI().swap(*meas.pcl_proc_next);   // 清空 pcl_proc_next
      ...
      {
        if (lid_header_time_buffer.front() > img_capture_time) break; // 雷达晚于图像
        auto pcl(lid_raw_data_buffer.front()->points);
        double frame_header_time(lid_header_time_buffer.front());
        float max_offs_time_ms = (m.lio_time - frame_header_time) * 1000.0f;
            /** max_offs_time_ms 求出在时间轴上图像时刻比雷达帧启始时刻滞后的时间*/
    ...
    case LIO://将图像数据与时间对齐的IMU数据打包
    {
      ...
      struct MeasureGroup m;
      m.vio_time = img_capture_time;          // 图像时间
      m.lio_time = meas.last_lio_update_time; // 上一次 LIO 更新完成的时间戳，用于时间对齐
      m.img = img_buffer.front();             // 图像
      mtx_buffer.lock();
      ...
      mtx_buffer.unlock();
      sig_buffer.notify_all();
      meas.measures.push_back(m);   /** 某帧图像 + 对应 IMU 序列 + 同步时刻戳*/
