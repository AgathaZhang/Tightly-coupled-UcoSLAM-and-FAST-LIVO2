...
void VIOManager::computeJacobianAndUpdateEKF(cv::Mat img)
{
  ...
  {
    if (inverse_composition_en)         //选择优化方法  07.23临时为了进updateStateInverse
    // if(true)
    {
      has_ref_patch_cache = false;
      updateStateInverse(img, level);   //反向传播 小运动情况下效果更好
    }
    else
...
{
  if (total_points == 0) return; // 如果没有匹配点就退出

  StatesGroup old_state = (*state); // 保存当前状态作为备份
  V2D pc;           // 表示当前三维点投影到图像平面后的 像素坐标（Pixel Coordinates）
  // MD(1, 2) Jimg;
  // MD(2, 3) Jdpi;
  // MD(1, 3) Jdphi, Jdp, JdR, Jdt; // 定义各种Jacobian项

  MD(1, 2) Jimg;    // 图像梯度雅克比，对应像素灰度对像素位置的导数 ∂I/∂u, ∂I/∂v
  MD(2, 3) Jdpi;    // 像素位置对3D点坐标的导数，即投影函数的雅克比 ∂π(P)/∂P，π: 3D → 2D
  MD(1, 3) Jdphi;   // 图像残差对空间点的导数，∂r/∂X（中间变量）
  MD(1, 3) Jdp;     // 图像残差对点位置 P 的导数（物理空间），∂r/∂P
  MD(1, 3) JdR;     // 图像残差对姿态（旋转）的导数，∂r/∂θ
  MD(1, 3) Jdt;     // 图像残差对平移的导数，∂r/∂t

  VectorXd z;       // 残差向量
  MatrixXd H_sub;   // 局部观测雅可比矩阵 H_sub.rows() = z.size()，H_sub.cols() = 6

  bool EKF_end = false; // 是否提前结束
  float last_error = std::numeric_limits<float>::max(); // 上一轮误差
  compute_jacobian_time = update_ekf_time = 0.0; // 初始化计时器
  M3D P_wi_hat; // 用于存储Pwi的反对称矩阵
  bool z_init = true;
  const int H_DIM = total_points * patch_size_total; // 测量总维度

  z.resize(H_DIM);
  z.setZero(); // 初始化残差向量
  H_sub.resize(H_DIM, 6);
  H_sub.setZero(); // 初始化观测Jacobian矩阵

  for (int iteration = 0; iteration < max_iterations; iteration++)
  {
    double t1 = omp_get_wtime();
    double count_outlier = 0;
    if (has_ref_patch_cache == false) precomputeReferencePatches(level); // 预计算参考图像patch
    int n_meas = 0;
    float error = 0.0;

    M3D Rwi(state->rot_end); // 当前相机旋转
    V3D Pwi(state->pos_end); // 当前相机位置
    P_wi_hat << SKEW_SYM_MATRX(Pwi); // 对Pwi构造反对称矩阵  将当前帧相机位置 Pwi 构造成一个用于叉乘计算的反对称矩阵，方便参与姿态扰动的雅可比推导

    Rcw = Rci * Rwi.transpose(); // 世界到相机的旋转
    Pcw = -Rci * Rwi.transpose() * Pwi + Pci; // 世界到相机的平移

    M3D p_hat;

    for (int i = 0; i < total_points; i++)
    {
      float patch_error = 0.0;        // 光度误差
      const int scale = (1 << level); // 图像金字塔层级缩放因子
      VisualPoint *pt = visual_submap->voxel_points[i];

      if (pt == nullptr) continue;

      V3D pf = Rcw * pt->pos_ + Pcw; // 转到图像系 世界坐标变换为当前帧相机坐标
      pc = cam->world2cam(pf);       // 投影到图像平面

      // 亚像素双线性插值准备
      const float u_ref = pc[0];                                      // 提取图像中点的u坐标（列）
      const float v_ref = pc[1];                                      // 提取图像中点的v坐标（行）
      const int u_ref_i = floorf(pc[0] / scale) * scale;              // 计算u方向最近的金字塔尺度对齐整数坐
      const int v_ref_i = floorf(pc[1] / scale) * scale;              // 计算v方向最近的金字塔尺度对齐整数坐标  表示在图像金字塔的当前层上，对齐到最近的整数像素位置
      const float subpix_u_ref = (u_ref - u_ref_i) / scale;           // u方向的亚像素偏移（0~1之间）
      const float subpix_v_ref = (v_ref - v_ref_i) / scale;           // v方向的亚像素偏移（0~1之间）
      const float w_ref_tl = (1.0 - subpix_u_ref) * (1.0 - subpix_v_ref); // 左上角像素的双线性插值权重
      const float w_ref_tr = subpix_u_ref * (1.0 - subpix_v_ref);         // 右上角像素的双线性插值权重
      const float w_ref_bl = (1.0 - subpix_u_ref) * subpix_v_ref;         // 左下角像素的双线性插值权重
      const float w_ref_br = subpix_u_ref * subpix_v_ref;                 // 右下角像素的双线性插值权重


      vector<float> P = visual_submap->warp_patch[i]; // 获取patch图像数据

      for (int x = 0; x < patch_size; x++)
      { // 用于获取图像中某个像素块左上角像素的指针，以便后续进行 patch 区域的光度值提取和插值计算
        uint8_t *img_ptr = (uint8_t *)img.data + (v_ref_i + x * scale - patch_size_half * scale) * width + u_ref_i - patch_size_half * scale;
        for (int y = 0; y < patch_size; ++y, img_ptr += scale)
        {
          // 双线性插值计算重投影图像值 - 参考值，得到残差
          ...
          MD(1, 3) J_dt = H_sub_inv.block<1, 3>(i * patch_size_total + x * patch_size + y, 3);    // H_sub_inv 在 precomputeReferencePatches(level); // 在这里构造
          JdR = J_dR * Rwi + J_dt * P_wi_hat * Rwi; // 链式求导转到世界系
          Jdt = J_dt * Rwi;
    ...
    error = error / n_meas; // 计算平均残差
    compute_jacobian_time += omp_get_wtime() - t1; // 记录Jacobian计算时间

    double t3 = omp_get_wtime();

    if (error <= last_error) // 残差下降，更新状态
    {
      old_state = (*state);
      last_error = error;
      ...
      H_T_H.block<6, 6>(0, 0) = H_sub_T * H_sub; // H^T H   是一个 6 × 6 的信息矩阵，也叫观测信息矩阵，表示当前观测对状态的约束强度
      MD(DIM_STATE, DIM_STATE) &&K_1 = (H_T_H + (state->cov / img_point_cov).inverse()).inverse();  // 用来求联合协方差  K = (Hᵀ * H + (P / σ²)⁻¹)⁻¹

      auto &&HTz = H_sub_T * z;   // 计算观测残差对状态的贡献项
      auto vec = (*state_propagat) - (*state);

      G.block<DIM_STATE, 6>(0, 0) = K_1.block<DIM_STATE, 6>(0, 0) * H_T_H.block<6, 6>(0, 0);        // 构造一个中间变量 G，用来修正先验项影响
      auto solution = -K_1.block<DIM_STATE, 6>(0, 0) * HTz + vec - G.block<DIM_STATE, 6>(0, 0) * vec.block<6, 1>(0, 0);

      (*state) += solution; // 应用状态更新
    ...
    if (iteration == max_iterations || EKF_end) break; // 满足条件则提前退出
  }
}
...
/** 是根据 IMU 状态（姿态 + 位置）和相机-IMU外参（Rci, Pci），更新当前图像帧在世界坐标系下的位姿 T_f_w_（SE3变换）*/
void VIOManager::updateFrameState(StatesGroup state)
{
  ...
  Rcw = Rci * Rwi.transpose();
  Pcw = -Rci * Rwi.transpose() * Pwi + Pci;
  new_frame_->T_f_w_ = SE3(Rcw, Pcw);   // 更新当前图像帧在世界坐标系的位姿
}
...
void VIOManager::dumpDataForColmap()
{
  ...
  if(cam_model_string == "EquidistantCamera"){

    if(camera_fisheye_first){
      camera_fisheye_first = false;

      Eigen::Matrix3d left_g_cam_K = Eigen::Matrix3d::Identity();
      left_g_cam_K(0,0) = cam->fx();
      left_g_cam_K(0,2) = cam->cx();
      left_g_cam_K(1,1) = cam->fy();
      left_g_cam_K(1,2) = cam->cy();
      cv::Mat left_intrinsic;
      cv::eigen2cv( left_g_cam_K, left_intrinsic );
      cv::Mat cv_left_distortionCoeffs = cv::Mat(4,1, cv::DataType<double>::type);
      cv_left_distortionCoeffs.at<double>(0,0) = fisheyeCamera->k1();
      cv_left_distortionCoeffs.at<double>(1,0) = fisheyeCamera->k2();
      cv_left_distortionCoeffs.at<double>(2,0) = fisheyeCamera->k3();
      cv_left_distortionCoeffs.at<double>(3,0) = fisheyeCamera->k4();
      //std::cout<<"k1:"<<fisheyeCamera->k1()<<" k2:"<<fisheyeCamera->k2()<<" k3:"<<fisheyeCamera->k3()<<" k4:"<<fisheyeCamera->k4()<<std::endl;

      cv::fisheye::initUndistortRectifyMap(left_intrinsic, cv_left_distortionCoeffs, cv::Mat(), left_intrinsic, cv::Size( img_rgb.cols / 1, img_rgb.rows / 1 ), CV_16SC2, m_left_ud_map1, m_left_ud_map2);//m_left_ud_map1,m_left_ud_map2   m_right_ud_map1,m_right_ud_map2
    }
   
    cv::remap( img_rgb, img_rgb_undistort, m_left_ud_map1, m_left_ud_map2, cv::INTER_LINEAR );
...
void VIOManager::processFrame(cv::Mat &img,                         // 当前帧图像
                        vector<pointWithVar> &pg,                   // 当前帧中所有投影点，（包含坐标及协方差等信息）
                        const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map,// 地图中体素坐标与体素树的映射 地图特征体素的八叉树结构（包含局部地图信息）
                        double img_time)                            // 当前图像的时间戳(相对于首次)
{
  ...
  new_frame_.reset(new Frame(cam, img));    // cam参数是 vikit（rpg_vikit）库 中对相机模型
  updateFrameState(*state);                 // 更新new_frame_->T_f_w_   IMU → 世界 → 相机 的坐标变换链
  resetGrid();                              // 重置 VIO 网格地图管理器的所有缓存和状态，为新一帧视觉处理准备干净的 voxel 数据结构与索引映射
/*
前段：对视觉点处理与点云融合
中段：体素地图维护
后段：视觉点回传确认视觉点准度
*/
  ...
  retrieveFromVisualSparseMap(img, pg, feat_map);/** 点云投影与视觉点提取，用于构建图像残差的“观测数据”*/
  // 从视觉稀疏地图中提取当前帧视野内的稀疏特征点，并构建稠密深度图和对应匹配补丁，为后续视觉惯性前端匹配和跟踪提供高质量初始观测
  double t2 = omp_get_wtime();
  /** 在不同金字塔层级上执行状态雅可比计算并更新 EKF 状态 */
  computeJacobianAndUpdateEKF(img);//计算扩展卡尔曼滤波（EKF）所需的雅可比矩阵，并通过 EKF 更新视觉惯性里程计（VIO）的状态

  // computeJacobianAndUpdateEKFbyUco(img);// TODO 构建 pose-graph 类型的残差

  double t3 = omp_get_wtime();
  /** 从当前图像和视觉稀疏图中提取特征点，构建新的视觉地图点，并插入体素地图中 */
...
/** 李航GPS观测*/
// bool UpdateGPS::update_gps_single(std::shared_ptr<State>& state, double curr_time, double process_prev_time) {
//     // 如果gps信息没有,那么不更新
//     if (state->_lidar_clones_IMU.find(curr_time) == state->_lidar_clones_IMU.end()) {
//         // LOG_ERROR("can't find imu pose at process lidar time {:03.3f}", sensor_time);
//         return false;
//     }
//     if (gps_queue_.size() < 2) {
//         return false;
//     }
//     std::deque<GNSS> select_gnss_data;
//     if (!get_gnss_datas(select_gnss_data, process_prev_time, curr_time)) {
//         // LOG_ERROR("can't find gnss delta_pose");
//         return false;
//     }
//     GNSS find_gnss_data = select_gnss_data.back();
//     Eigen::Vector3d measurement_gps = find_gnss_data.enu_;
//     // 找到了同一个时刻的lio数据和gps数据
//     // LOG_INFO("curr_time:{}, gps_time:{}", curr_time, find_gnss_data.timestamp);
//     std::shared_ptr<PoseJPL> clone_Ij = state->_lidar_clones_IMU.at(curr_time);
//     PoseTrans T_world_to_lio(state->T_lio_world->Rot(), state->T_lio_world->pos());

//     // 代优化的变量为clone_Ij, 以及T_lio_world,12维
//     size_t max_hx_size = 2 * 6;
//     // 残差是平移的3维变量
//     size_t max_meas_size = 3;
//     std::vector<std::shared_ptr<Type>> Hx_order_big;
//     std::shared_ptr<PoseJPL> T_lio_world = state->T_lio_world;
//     Hx_order_big.push_back(clone_Ij);
//     Hx_order_big.push_back(T_lio_world);
//     // 残差是3维的
//     Eigen::VectorXd res_big = Eigen::VectorXd::Zero(max_meas_size);
//     // H 矩阵是 3 x 12
//     Eigen::MatrixXd Hx_big = Eigen::MatrixXd::Zero(max_meas_size, max_hx_size);
//     Eigen::Matrix3d R_GtoI = clone_Ij->Rot();
//     Eigen::Vector3d p_IinG = clone_Ij->pos();
//     // 转到odom坐标系
//     Eigen::Vector3d p_GPSinG = R_GtoI.transpose() * p_GPSinI + p_IinG;
//     // 转到enu坐标系
//     Eigen::Vector3d P_GPSinWorld = T_world_to_lio.R.transpose() * p_GPSinG + T_world_to_lio.t;
//     // res_big.block<3, 1>(0, 0) = -(P_GPSinWorld - measurement_gps);
//     // res_big.block<3, 1>(0, 0) = -(T_world_to_lio.R.transpose() * p_GPSinG + T_world_to_lio.t - measurement_gps);
//     // 残差的计算 观测-h(x)
//     res_big.block<3, 1>(0, 0) = -(T_world_to_lio.R.transpose() * R_GtoI.transpose() * p_GPSinI +
//                                   T_world_to_lio.R.transpose() * p_IinG + T_world_to_lio.t - measurement_gps);
//     // LOG_INFO("res_start:{}, res_norm:{}", res_big.transpose(), res_big.norm());
//     // de / d(clone_Ij_r)
//     Hx_big.block<3, 3>(0, 0) = -T_world_to_lio.R.transpose() * R_GtoI.transpose() * skew_x(p_GPSinI);
//     // de / d(clone_Ij_t)
//     Hx_big.block<3, 3>(0, 3) = T_world_to_lio.R.transpose();
//     // de / d(t_lio_world_r)
//     Hx_big.block<3, 3>(0, 6) = -T_world_to_lio.R.transpose() * skew_x(p_GPSinG);
//     // de / d(t_lio_world_t)
//     Hx_big.block<3, 3>(0, 9).setIdentity();

//     // gps噪声
//     Eigen::MatrixXd R_big = Eigen::MatrixXd::Identity(max_meas_size, max_meas_size);
//     // double gps_sigma_rot = 0.1;
//     // double gps_sigma_trans = 0.01;
//     R_big.block<3, 3>(0, 0) *= gps_sigma;

//     StateHelper::EKFUpdate(state, Hx_order_big, Hx_big, res_big, R_big);

//     // res_big.block<3, 1>(0, 0) = -(P_GPSinWorld - measurement_gps);
//     // LOG_INFO("res_end:{}, res_norm:{}", res_big.transpose(), res_big.norm());
//     return true;
// }