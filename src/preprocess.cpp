...
/** 将原始雷达点云消息进行预处理：按线束编号分层存储，计算时间戳差值为曲率，筛选特征点并提取边缘/平面特征，供后续SLAM模块使用*/
void Preprocess::avia_handler_mid360(const sensor_msgs::PointCloud2::ConstPtr &msg){
  ...
  if (feature_enabled)
  {
    for (uint i = 1; i < plsize; i++)
    {
      ...
      if((pl_orig.points[i].line < N_SCANS) && ((pl_orig.points[i].tag & 0x30) == 0x10 || (pl_orig.points[i].tag & 0x30) == 0x00))
      {
        ...
        pl_full[i].curvature = (pl_orig.points[i].timestamp / double(1e9)-begin_time)*1000;//pl_orig.points[i].timestamp / float(1000000); // use curvature as time of each laser points
  ...
  else
  {
    for (uint i = 0; i < plsize; i++)
    {
      if ((pl_orig.points[i].line < N_SCANS)) // && ((msg->points[i].tag & 0x30) == 0x10))
      {
        ...
        pl_full[i].z = pl_orig.points[i].z;
        pl_full[i].intensity = pl_orig.points[i].intensity;
        pl_full[i].curvature = (pl_orig.points[i].timestamp / double(1e9)-begin_time)*1000;//pl_orig.points[i].offset_time / float(1000000); // use curvature as time of each laser points
