...
void ImuProcess::Process2(LidarMeasureGroup &lidar_meas,    // 雷达测量
                          StatesGroup &stat,                // 状态序列
                          PointCloudXYZI::Ptr cur_pcl_un_)  // 去畸变点云帧
{
  double t1, t2, t3;
  t1 = omp_get_wtime();
  ...
  { //时间对齐 计算点云结束时间
    ...
    if (meas.imu.empty()) { return; };     //imu是空的直接返回
    ...
    last_imu = meas.imu.back();
    //init_iter_num为帧数  所以间隔为20帧开始重新初始化//init_iter_num为帧数  所以间隔为20帧开始重新初始化
  ...
  UndistortPcl(lidar_meas, stat, *cur_pcl_un_);/** 真正预计分是在这个里面做的 和点云去畸变一起*/
