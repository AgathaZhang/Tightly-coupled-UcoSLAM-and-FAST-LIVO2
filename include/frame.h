...
public:
  ...
  static int frame_counter_; //!< 已创建帧的数量，用于设置唯一的帧ID
  int id_;                   //!< 当前帧的唯一ID
  vk::AbstractCamera *cam_;  //!< 相机模型指针
  SE3 T_f_w_;                //!< 当前帧到世界坐标系的变换（T_frame_from_world）
  SE3 T_f_w_prior_;          //!< 来自IMU预测的先验帧位姿
  cv::Mat img_;              //!< 当前帧的图像数据
  Features fts_;             //!< 当前帧图像中的特征点列表
